...
export class AuthService implements IAuthService {
  ...
  async signup(signupData: SignupDto): Promise<AuthReturnDto> {
    ...
    const tokenMock =
      "hgjfdsjf3ghtrh45.h5h54hg54g54g54gh54h.h5445h5h454h45h54h";

    const user = await this.userRepository.findByEmail(email);

    if (!user) {
      throw new ForbiddenError(
        "You must use the same email that was used to signup"
      );
    }
  ...
  async code(ResetPasswordDto: ResetPasswordDto): Promise<void> {
    ...
    if (!code) {
      throw new ForbiddenError(
        "You must provide a code to reset your password"
      );
    }
    ...
    await this.userRepository.save(
      plainToInstance(UserDto, { ...user, password })
    );
  }
}
