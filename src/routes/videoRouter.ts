...
const createVideoRouter = () => {
  ...
  router.post("/:id/comments", allUserAuthMiddleware, errorMiddleware((req, res) => getController().sendComment(req, res)));
  router.get("/:id/comments", allUserAuthMiddleware, errorMiddleware((req, res) => getController().findComments(req, res)));
